...
int Hr_Z_Lvl;
int Hr_Z0 = 1; // No HR
int Hr_Z1 = HrMaxVal * 0.5; // Blue
int Hr_Z2 = HrMaxVal * 0.6; // Green
...
void loop() {
  ...
  if (connected) {
   ...
   // setup HR zones as a percentage of Max HR
    Hr_Z0 = 1; // No HR
    Hr_Z1 = HrMaxVal * 0.5; // Blue
    Hr_Z2 = HrMaxVal * 0.6; // Green
    ...
    if (LEDPos <= Hr_Z0){                   //  Off
      ...
      strip.clear();
      strip.fill(LED_Green,0,LEDPos);        //  Set pixel's color BLUE
      strip.show();                       //  Update strip to match
      Hr_Z_Lvl = 2;
    }
    else if ((HrInt >= Hr_Z2) && (HrInt <= Hr_Z3) ){
      strip.clear();
      strip.fill(LED_Yellow,0,LEDPos);        //  Set pixel's color (in RAM)
      strip.show();                       //  Update strip to match
      Hr_Z_Lvl = 3;
    }
    else if ((HrInt >= Hr_Z3) && (HrInt <= Hr_Z4) ){
      strip.clear();
      strip.fill(LED_Orange,0,LEDPos);        //  Set pixel's color (in RAM)
      strip.show();                       //  Update strip to match
      Hr_Z_Lvl = 4;
    }
    else if (HrInt >= Hr_Z4 ){
      strip.clear();
      strip.fill(LED_Red,0,LEDPos);        //  Set pixel's color (in RAM)
      strip.show();                       //  Update strip to match
      Hr_Z_Lvl = 5;
    }
